...
def push_question_id_to_label(base_url, name, question_ids, env="bj-prod-221"):
    """将需要标注的case推送到标注平台"""
    url = base_url + "/v1/label/manual_assign"
    payload = {
        "name": name,
        "env": env,
        "question_ids": question_ids
    }
    res = requests.request(method="POST", url=url, json=payload, verify=False)
...
def badcase_tagging_push(start_time=None, end_time=None, exclude_domain=None, exclude_intent=None):
    ...
    result = push_question_id_to_label(base_url="http://annotaion.region-dev-1.service.iamidata.com:31123",
                                       name=task_name, question_ids=task_question_ids, env="bj-prod-221")
